...
class RegressionMetric(CometModel):
    ...
    def read_csv(self,train_data_path: str, validation_data_path: str):
        ...
        raw_data = []
        for line in open('./data/train_all_final.jsonl', 'r'):
            raw_data.append(json.loads(line))
        
        mapper={'Yes':1,'No':0}
        sum_ref_data=[]
        for i in range(len(raw_data)):
            if raw_data[i]['question']=='question: Is this summary relevant to the reference? ':
                summary=raw_data[i]['summary']
                reference = raw_data[i]['reference']
                document =  raw_data[i]['document']
                label=mapper[raw_data[i]['label']]
                sum_ref_data.append({'document':document,'reference':reference,'summary': summary, 'label':label})
        random.shuffle(sum_ref_data)
        

        sum_doc_data=[]
        for i in range(len(raw_data)):
            if raw_data[i]['question']=='question: Is this a coherent summary to the document? ':
                summary=raw_data[i]['summary']
                reference = raw_data[i]['reference']
                document =  raw_data[i]['document']
                label=mapper[raw_data[i]['label']]
                sum_doc_data.append({'document':document,'reference':reference,'summary': summary, 'label':label})
        random.shuffle(sum_doc_data)
        
        sum_doc_ref_data=[]
        sum_doc_ref_data.extend(sum_ref_data[0:int(len(sum_ref_data)/2)])
        sum_doc_ref_data.extend(sum_doc_data[0:int(len(sum_doc_data)/2)])
        random.shuffle(sum_doc_ref_data)
        
        
        print(len(sum_ref_data))
        print(len(sum_doc_data))
        
        return {
            "train_data_for_sum_doc": sum_doc_data,
            "train_data_for_sum_ref": sum_ref_data,
