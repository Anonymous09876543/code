...
class PrefixRobertaEncoder(RobertaPreTrainedModel):
    ...
    def get_prompt(self,task, batch_size):
        ...
        if task=='hyp-src':
            T=list(range(0,self.pre_seq_len))
        elif task=='hyp-ref':
            T=list(range(self.pre_seq_len-1,-1,-1))
        elif task == 'hyp-src-ref':
            T1=list(range(1,self.pre_seq_len,2))
            T2=list(range(0,self.pre_seq_len,2))
            T1.extend(T2)
            T=T1
       

        
        self.prefix_tokens = torch.Tensor(T).long().to(self.model.device)
        #print(task,self.prefix_tokens)
