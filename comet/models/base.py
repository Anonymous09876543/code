...
class CometModel(ptl.LightningModule, metaclass=abc.ABCMeta):
    ...
    :param pool: Pooling strategy to derive a sentence embedding ['cls', 'max', 'avg', 'avg_each'].
    ...
    def __init__(
        ...
        self.test_result = { 'hyp-ref': [],'hyp-src': [], 'coherence': [], 'consistency': [], 'fluency': [], 'relevance': []}
        self.cur_epoch=0
    ...
    def compute_loss(
        self, predictions: Dict[str, torch.Tensor]) -> torch.Tensor:

        #pos = torch.exp(score[:self.batch_size, ])
        #neg = torch.sum(torch.exp(score[batch_size:, ].view(batch_size, self.hparams.negative_sample_number)), dim=1).unsqueeze(dim=1)
        #loss = -torch.log(pos / (pos + neg))
        
        score_matrix = predictions["score"].view(-1, self.hparams.neg_sample_number+1)
        score_matrix = torch.exp(score_matrix)
        return torch.sum(-torch.log(score_matrix[:, 0] / torch.sum(score_matrix, dim=1)))
    ...
    def get_sentence_embedding(
        ...
        origin_attention_mask: torch.Tensor = None,
        token_type_ids: torch.Tensor = None,
        token_type_masks: torch.Tensor = None,
        cls_token_ids: torch.Tensor = None,
        ...
        """
        encoder_out = self.encoder(task=input_segments,batch_size=self.hparams.batch_size,input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, position_ids=position_ids)
        ...
        if self.layerwise_attention: 
            ...
            if not self.training:
                ...
                for split in range(n_splits):
                    ...
                    split_attn = torch.split(origin_attention_mask, 8)[split] if origin_attention_mask is not None else torch.split(attention_mask, 8)[split]
                    embeddings.append(self.layerwise_attention(all_layers, split_attn))
                embeddings = torch.cat(embeddings, dim=0)
            else:
                embeddings = self.layerwise_attention(
                    encoder_out["all_layers"], origin_attention_mask if origin_attention_mask is not None else attention_mask
                )
        ...
        if self.hparams.pool == "default":
            ...
            sentemb = average_pooling(
                input_ids,
                embeddings,
                origin_attention_mask if origin_attention_mask is not None else attention_mask,
                self.encoder.tokenizer.pad_token_id,
            )
        
        elif self.hparams.pool == "avg_each":
            sentemb = average_each_pooling(
                input_ids,
                embeddings,
                origin_attention_mask if origin_attention_mask is not None else attention_mask,
                self.encoder.tokenizer.pad_token_id,
        ...
        elif self.hparams.pool == "cls_each":
            sentemb = class_each_pooling(
                input_ids,
                embeddings,
                origin_attention_mask if origin_attention_mask is not None else attention_mask,
                self.encoder.tokenizer.pad_token_id,
                cls_token_ids,
                num_of_inputs,
                self.encoder.model.embeddings.position_ids
            )
    ...
    def training_step(
        ...
        batch_prediction = self.forward(input_segments='hyp-ref',**batch_input)
        loss_value = self.my_loss(batch_prediction['score'],target["score"])
        ...
        for input_segments, batch_item in batch.items():
            ...
            loss = self.my_loss(batch_prediction['score'],batch_target["score"])
        ...
        if  self.nr_frozen_epochs < 1.0     and self.nr_frozen_epochs > 0.0    and batch_nb > self.epoch_total_steps * self.nr_frozen_epochs:
            self.unfreeze_encoder()
            self._frozen = False

        self.log_dict(all_loss_value, prog_bar=True)
        return torch.Tensor([sum(all_loss_value.values()) / len(all_loss_value)])
    ...
    def validation_step(
        ...
        self.test_result['coherence'].extend(batch_target['coherence'])
        self.test_result['consistency'].extend(batch_target['consistency'])
        self.test_result['fluency'].extend(batch_target['fluency'])
        self.test_result['relevance'].extend(batch_target['relevance'])

        task_list = ['hyp-ref','hyp-src']
        for input_segments in task_list:
            batch_prediction = self.forward(input_segments=input_segments, **batch_input)
            output = F.softmax(batch_prediction['score'],dim=1)
            self.test_result[input_segments].extend(output[:,1].view(-1).tolist())
            
           


    def validation_epoch_end(self, *args, **kwargs) -> None:
        # computes & output metrics
        output = pd.DataFrame(self.test_result)
        kendall_metrics = output.corr(method='kendall')[['coherence', 'consistency', 'fluency', 'relevance']][0:1]
        pearson_metrics = output.corr(method='pearson')[['coherence', 'consistency', 'fluency', 'relevance']][0:1]
        spearman_metrics = output.corr(method='spearman')[['coherence', 'consistency', 'fluency', 'relevance']][0:1]

        result = kendall_metrics.append(pearson_metrics)
        result = result.append(spearman_metrics)
        result.to_csv('./result/epoch'+str(self.cur_epoch)+'_metric.csv')
        output.to_csv('./result/epoch'+str(self.cur_epoch)+'_scores.csv')
        self.cur_epoch+=1

        values = {'val_loss': (kendall_metrics.mean().mean()+pearson_metrics.mean().mean()+spearman_metrics.mean().mean())/3,
                  'kendall_metrics': kendall_metrics.mean().mean(),
                  'pearson_metrics': pearson_metrics.mean().mean(),
                  'spearman_metrics': spearman_metrics.mean().mean(),
        }
        self.log_dict(values, prog_bar=True)

        # update test_result & negative sample again
        self.test_result = { 'hyp-ref': [],'hyp-src': [], 'coherence': [], 'consistency': [], 'fluency': [], 'relevance': []}
    ...
    def setup(self, stage) -> None:
        ...
        if stage in (None, "fit"):
            ...
            data = self.read_csv(self.hparams.train_data, self.hparams.validation_data)
            self.train_dataset=data
            #print(self.train_dataset[0].keys())
            ...
            self.validation_dataset = data['validation_data']
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        """
        dataloader_sum_doc = DataLoader(
              dataset=self.train_dataset['train_data_for_sum_doc'],
              sampler=RandomSampler(self.train_dataset['train_data_for_sum_doc']),
              batch_size=self.hparams.batch_size,
              collate_fn=self.prepare_sample,
              num_workers=min(4, multiprocessing.cpu_count()),
        )

        dataloader_sum_ref = DataLoader(
              dataset=self.train_dataset['train_data_for_sum_ref'],
              sampler=RandomSampler(self.train_dataset['train_data_for_sum_ref']),
              batch_size=self.hparams.batch_size,
              collate_fn=self.prepare_sample,
              num_workers=min(4, multiprocessing.cpu_count()),
        )
        ...
        dataloaders_dict = {'hyp-src': dataloader_sum_doc, 'hyp-ref': dataloader_sum_ref}
    ...
    def val_dataloader(self) -> DataLoader:
        ...
        dataloader=DataLoader(
            ...
            batch_size=self.hparams.batch_size,
            collate_fn=self.prepare_valid_sample,
            num_workers=min(4, multiprocessing.cpu_count()),
        )
        return dataloader
